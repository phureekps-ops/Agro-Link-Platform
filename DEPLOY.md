...
grant_logistics_marketplace.sql
```


**เพิ่มเมื่อ 2026-09-26:** `grant_logistics_marketplace.sql` — ตามคำขอ
"สร้างพอร์ทัล Logistics และให้มีโปรแกรมพื้นฐานเกี่ยวกับธุรกิจนี้" (รถบรรทุก/
รถพ่วงขนาดใหญ่ขนถ่ายผลผลิต ปุ๋ย ชีวมวล — มีการจอง ต่อรองราคา ประกาศหางาน
รถเที่ยวเปล่า) — ขยายพอร์ทัลผู้ให้บริการขนส่ง (`frontend/logistics/`) ที่มีอยู่
แล้ว (เดิมมีแค่ระบบติดตามงานที่สหกรณ์มอบหมายให้ — บันทึกออกเดินทาง/หลักฐาน
ส่งมอบ/รายงานปัญหา) เพิ่ม 4 ความสามารถใหม่ที่ `grant_logistics_portal.sql`
เดิม เคยทิ้งไว้เป็น "Follow-up work" อย่างชัดเจนว่ายังไม่มี:

1. **ทะเบียนรถบรรทุก/รถพ่วงของตัวเอง** — ตารางใหม่ `logistics.carrier_vehicle`
   (แยกจาก `logistics.vehicle` เดิมที่เป็นข้อมูลภายในของฝั่งสหกรณ์)
2. **พื้นที่ให้บริการ** — ใช้คอลัมน์ `partner.vendor_profile.service_regions`
   ที่มีอยู่แล้ว (ทุกองค์กรมีติดตัวมาตั้งแต่สมัครสมาชิก) แบบเดียวกับ
   `GET/PUT /machinery/service-regions` ทุกประการ ไม่มีตารางใหม่
3. **กระดานงานขนส่ง / เสนอราคา / ยืนยันการจอง** — **ไม่ได้สร้างระบบประมูลใหม่**
   แต่ขยายตลาดขอใบเสนอราคา (RFQ) กลาง `procurement.rfq`/`procurement.rfq_quote`
   ที่ใช้งานจริงอยู่แล้วร่วมกับพอร์ทัลอื่น (สหกรณ์/ผู้รับซื้อ/ผู้ขายปัจจัยการผลิต)
   ให้รองรับหมวดหมู่ใหม่ `logistics_transport` — สหกรณ์/ผู้ซื้อประกาศงานขนย้าย
   ผลผลิต/ปุ๋ย/ชีวมวล ผู้ให้บริการขนส่งแข่งขันเสนอราคา เมื่อผู้ประกาศ "ยอมรับ"
   ใบเสนอราคา ระบบสร้างสัญญาบริการ (`contract.contract`, ประเภท
   `service_agreement`) ให้อัตโนมัติเหมือนงาน RFQ ประเภทอื่น
4. **กระดานรถเที่ยวเปล่า** — ตารางใหม่ `logistics.empty_leg` (แนวคิดกลับด้านจาก
   RFQ: ผู้ขนส่งเป็นฝ่ายประกาศว่ามีที่ว่าง ไม่ใช่ฝ่ายขอ) เปิดให้**ทุกองค์กร**
   เรียกดูกระดานได้ (ไม่ใช่แค่ผู้ให้บริการขนส่งด้วยกัน) เพราะจุดประสงค์คือให้
   สหกรณ์/ผู้ซื้อที่มีของต้องขนหาเที่ยวรถราคาถูกเจอ — การจองยังทำนอกระบบ
   โดยตรงกับผู้ให้บริการขนส่ง ยังไม่ผูกกับระบบยืนยันการจองอัตโนมัติ

งานที่สหกรณ์มอบหมายให้โดยตรง (dispatch/POD/exception เดิม) **ไม่ถูกแก้ไข
เลย** — ความสามารถใหม่ทั้ง 4 อย่างเป็นส่วนเสริมคู่ขนาน ดูรายละเอียดการออกแบบ
ทั้งหมด (เหตุผลที่เลือกวิธีนี้แทนการสร้างระบบคู่ขนาน) ที่คอมเมนต์หัวไฟล์
migration นี้เอง

เพิ่มหน้าเอกสารสาธารณะใหม่ `frontend/logistics-portal.html` อธิบายภาพรวม
ทั้งหมด พร้อมลิงก์จาก footer คอลัมน์ "พันธมิตร" ทุกหน้า (21 หน้า) และลิงก์
เชื่อมจากเนื้อหา `custom-fertilizer.html`/`biomass-economy.html` (ส่วนที่พูด
ถึงการขนย้ายปุ๋ย/ชีวมวลปริมาณมาก)

**หมายเหตุสำคัญ:** migration นี้ยังไม่ได้รันกับฐานข้อมูลจริงบน Render —
ต้องรันเองผ่าน psql ตามขั้นตอนใน DEPLOY.md ข้อ 5-7 ก่อนความสามารถใหม่ทั้ง 4
อย่างข้างต้นจะใช้งานได้ (ระบบงานที่สหกรณ์มอบหมายให้โดยตรงยังใช้งานได้ปกติ
โดยไม่ต้องรัน migration นี้ก่อน เพราะไม่ได้แตะตารางเดิมเลย)
