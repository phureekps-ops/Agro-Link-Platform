...
รายการที่ 12 ในตารางราคาบริการเครื่องจักรกล (เดิมมี 11 รายการ) — เป็นการให้
เช่าอุปกรณ์เปล่า (ผู้ให้บริการแค่ปล่อยเช่าตัวเซนเซอร์ ไม่มีการลงมือทำงานเหมือน
รายการอื่นๆ) เหมือนกับ machinery_rental ที่มีอยู่แล้ว จึงใช้ service_type เดิม
คือ `other` ไม่ต้องเพิ่มหมวดใหม่ และไม่ต้องแก้ dropdown ตัวกรองใดๆ ในหน้า
frontend เลย (เหตุผลเดียวกับตอนเพิ่ม machinery_rental) — ขยายแค่ CHECK
constraint ของ service_key เท่านั้น หน้าฟอร์มตั้งราคาของพอร์ทัลเครื่องจักรกล
(`frontend/machinery/js/dashboard.js`) ดึงรายการราคามาแสดงจาก
`GET /machinery/rate-card` แบบไดนามิกอยู่แล้ว จึงไม่ต้องแก้ไฟล์ frontend เลย
สักไฟล์ — รายการใหม่นี้จะโผล่ในฟอร์มอัตโนมัติทันทีที่รัน migration นี้แล้ว
(หมายเหตุ: เซนเซอร์นี้วัดค่าน้ำแบบเดียวกับที่ใช้ประเมิน AWD ในโมดูลคาร์บอน
เครดิต แต่รายการนี้เป็นแค่การให้เช่าอุปกรณ์ในพอร์ทัลเครื่องจักรกล ไม่เกี่ยวข้อง
และไม่แตะตารางหลักฐาน AWD ของโมดูลคาร์บอนเครดิตแต่อย่างใด)
...
**เพิ่มเมื่อ 2026-08-29 (ดึกที่สุด, frontend+backend เท่านั้น ไม่มี migration):**
หน้าเช่า/จองบริการเครื่องจักรกล (`frontend/machinery-marketplace.html`) เพิ่ม
การกรองผู้ให้บริการตามจังหวัดได้จริง เหมือนที่ทำให้ตลาดปัจจัยการผลิตไปก่อนหน้านี้
ในรอบเดียวกัน — ต่างกันตรงที่หน้านี้ไม่เคยมีดรอปดาวน์จังหวัดมาก่อนเลย จึงต้องเพิ่ม
ใหม่ทั้งหมด (`GET /farmer/machinery-providers` เพิ่ม query param `province_code`
ใหม่) และฝั่งผู้ให้บริการเครื่องจักรกล/ลานตากก็ไม่เคยมี route `service-regions`
มาก่อนเลย (ต่างจาก InputSupplier ที่มี route อยู่แล้วแต่ไม่มีหน้าเว็บใช้) จึงต้อง
สร้าง `GET`/`PUT /machinery/service-regions` ขึ้นใหม่ทั้งคู่ (โครงสร้างเดียวกับ
`/inputsupplier/service-regions` ทุกประการ) พร้อมเพิ่มหน้า "พื้นที่ให้บริการ
(จังหวัด)" ในแดชบอร์ดผู้ให้บริการ (`frontend/machinery/dashboard.html`) — ใช้
ระบบรหัสจังหวัดแบบ ISO 3166-2:TH เดิม (`frontend/js/provinces.js`) เดียวกัน
**ไม่มีการเพิ่มระดับ "อำเภอ"** ในรอบนี้เช่นกัน ด้วยเหตุผลเดียวกับตลาดปัจจัยการผลิต
ข้างต้น (ไม่มีข้อมูลอำเภอในระบบเลย) ไม่มี migration SQL ใดๆ เพราะ
`partner.vendor_profile.service_regions` เป็นคอลัมน์ที่มีอยู่แล้วตั้งแต่เดิม
...
**เพิ่มเมื่อ 2026-08-29 (ระดับอำเภอ, frontend+backend เท่านั้น ไม่มี migration):**
ขยายการกรองตามพื้นที่ให้ลึกถึงระดับ "อำเภอ" ทั้งสองตลาด (ตลาดปัจจัยการผลิตและ
ตลาดเช่า/จองบริการเครื่องจักรกล) ต่อจากรอบ "ดึกมาก"/"ดึกที่สุด" ข้างต้นที่ทำได้
แค่ระดับจังหวัด — `GET /farmer/products`, `GET /farmer/input-suppliers`, และ
`GET /farmer/machinery-providers` ทั้งสามตัวเพิ่ม query param `district_code`
ใหม่ (ใช้คู่กับ `province_code` เดิมเสมอ ไม่ส่ง district_code เดี่ยวๆ) ผ่าน
ฟังก์ชันช่วยกรองร่วม `serviceRegionCondition()` ที่จำลองซ้ำไว้ในทั้ง `farmer.js`
และ `farmermachinery.js` (ตามธรรมเนียมเดิมของโปรเจกต์ที่ไม่แชร์โค้ดข้ามไฟล์
route) — ตรรกะ: ถ้าผู้ขาย/ผู้ให้บริการประกาศ "ทั้งจังหวัด" (รหัสจังหวัดเปล่าๆ เช่น
`TH-50`) ก็ตรงกับทุกอำเภอในจังหวัดนั้น, ถ้าประกาศเฉพาะบางอำเภอ (รหัส
`TH-50-01` เป็นต้น) ก็ตรงเฉพาะอำเภอนั้นหรือเฉพาะตอนที่เกษตรกรกรองอำเภอนั้น
พอดี, และ `service_regions` ว่างเปล่ายังคงหมายถึง "ให้บริการทุกที่" เหมือนเดิม

ฝั่งหน้าเว็บเกษตรกร (`frontend/marketplace.html`,
`frontend/machinery-marketplace.html`) เพิ่ม `<select id="districtFilter">`
คู่กับ `provinceFilter` เดิม — เปลี่ยนจังหวัดแล้ว dropdown อำเภอจะรีเซ็ตและ
กรองใหม่ให้เหลือเฉพาะอำเภอของจังหวัดนั้น ฝั่งแดชบอร์ดผู้ให้บริการทั้งสองฝั่ง
(`frontend/inputsupplier/dashboard.html` + `js/dashboard.js` และ
`frontend/machinery/dashboard.html` + `js/dashboard.js`) เปลี่ยนจากรายการ
checkbox จังหวัดล้วนๆ เป็นโครงสร้าง `<details><summary>` ต่อจังหวัด (ยุบไว้
ก่อนโดยปริยาย) ที่ข้างในมี checkbox ระดับอำเภอซ้อนอยู่ — ผู้ให้บริการติ๊กได้ทั้ง
ระดับจังหวัดและ/หรือระดับอำเภอในจังหวัดเดียวกันพร้อมกันโดยไม่บังคับว่าต้อง
สัมพันธ์กัน (เก็บลง `service_regions` เดียวกันแบบผสมกันได้อิสระ)

**ข้อมูลอ้างอิงอำเภอ (`frontend/js/districts.js`, ไฟล์ใหม่, ~928 อำเภอทั่ว
ประเทศ) มีข้อควรระวังสำคัญที่ต้องแจ้งไว้ตรงนี้:** ไม่มีรหัสอำเภอที่เป็นทางการจาก
รัฐบาลไทยหรือ ISO ให้อ้างอิง (มีแค่ระดับจังหวัดที่มีรหัส ISO 3166-2:TH) จึงต้อง
คิดรูปแบบรหัสขึ้นเอง (`<รหัสจังหวัด>-NN` เช่น `TH-50-01`, NN เป็นเลขที่กำหนด
เรียงเองในโปรเจกต์นี้ **ไม่ใช่รหัสภูมิศาสตร์ TIS-1099 ที่รัฐบาลไทยใช้จริง**)
และรายชื่ออำเภอทั้ง 928 รายการก็ **รวบรวมจากความรู้ทั่วไป/ความรู้ที่มีอยู่เดิมของ
ผู้ช่วย ไม่ได้ดึงจากแหล่งข้อมูลสดที่ตรวจสอบได้จริง** — ลองดึงจาก
`kongvut/thai-province-data` และ `DevelopedbyWill/thailand-canonical-admin-names`
บน GitHub แล้วแต่เครื่องมือเว็บติด provenance/robots.txt เข้าไม่ถึงไฟล์ข้อมูลดิบ
ได้ และตาราง Wikipedia ที่มีครบ 878 อำเภอ (ไม่รวม 50 เขตกรุงเทพฯ) ก็ใหญ่เกินกว่า
จะดึงมาครบทุกแถวโดยไม่เสี่ยงตกหล่น/สะกดผิดผ่านตัวสรุปเนื้อหาเว็บ — ได้แจ้งผู้ใช้
ถึงทางเลือกนี้และผู้ใช้ตอบยืนยันให้ทำแบบนี้ต่อ ("ให้ผมรวบรวมจากความรู้เดิม")
ผลรวมจำนวนอำเภอต่อจังหวัดบวกกันได้ 928 พอดี (ตรงกับตัวเลขที่ README ของ
`DevelopedbyWill/thailand-canonical-admin-names` อ้างไว้) ซึ่งเป็นแค่การเช็ค
ผลรวมคร่าวๆ **ไม่ได้ยืนยันความถูกต้องรายแถว** — หากพบชื่ออำเภอผิด/ตกหล่น
ให้แก้ไขตรงที่ไฟล์ `frontend/js/districts.js` ไฟล์เดียว (เป็น single source of
...
**เพิ่มเมื่อ 2026-08-30 (ตรวจสอบข้อมูล ไม่มีการแก้โค้ด/migration):** ผู้ใช้ขอให้
"หารายชื่อจังหวัดและอำเภอล่าสุด" เพื่อตรวจสอบไฟล์ `frontend/js/districts.js`
(ที่ตอนสร้างครั้งแรกรวบรวมจากความรู้ทั่วไปของผู้ช่วย ไม่ได้ดึงจากแหล่งข้อมูลสด
ตามที่ระบุไว้ในหมายเหตุด้านบน) — ทำการตรวจสอบ 2 ชั้น:

1. ยืนยันจำนวนรวมกับข้อมูลทางการของกรมการปกครอง (DOPA) ณ วันที่ 10 กุมภาพันธ์
   2568: 76 จังหวัด (ไม่รวมกรุงเทพฯ) + กรุงเทพฯ = 77, และ 878 อำเภอ + 50 เขต
   กรุงเทพฯ = 928 — ตรงกับตัวเลขที่มีอยู่ในไฟล์เป๊ะ ไม่มีจังหวัด/อำเภอใหม่ตั้งแต่
   จังหวัดบึงกาฬ (2554)
2. ตรวจสอบรายชื่ออำเภอครบทั้ง 77 จังหวัด เทียบกับ Wikipedia ภาษาอังกฤษ
   (แบ่งงานให้ subagent 6 ตัวทำพร้อมกัน จังหวัดละ ๆ) — **ผลคือไม่พบข้อผิดพลาด
   เลยสักจังหวัดเดียว** ทั้งจำนวน ชื่อ และลำดับ (มีข้อจำกัดของการตรวจสอบที่ต้อง
   บันทึกไว้ตรงๆ: ตาราง Wikipedia ระดับจังหวัดหลายจังหวัดดึงข้อมูลแถวไม่ได้ครบ
   ผ่านเครื่องมือดึงเว็บ ต้องอาศัยจำนวนที่ระบุในบทความ + สุ่มตรวจชื่อรายอำเภอที่
   เสี่ยงผิดสูงแทน และเครื่องมือดึงเว็บบางครั้งแปลงตัวอักษรไทยที่ดึงมาผิดเพี้ยน
   จึงต้องตรวจข้ามผ่านชื่อทับศัพท์ภาษาอังกฤษแทนการเชื่อข้อความไทยที่ดึงมาตรงๆ —
   รายละเอียดเต็มอยู่ในหมายเหตุที่อัปเดตแล้วในหัวไฟล์ `districts.js` เอง)

สรุป: ไฟล์ `frontend/js/districts.js` เดิมถูกต้องแล้ว ไม่ต้องแก้ไขข้อมูลใดๆ
...
**เพิ่มเมื่อ 2026-09-06 (รอบที่ 4 — ขยายสโคป ไม่มี migration ใหม่):**
ผู้ใช้ส่งข้อความตามมาว่า "ส่วนผู้ปล่อยกู้/กองทุนหมู่บ้าน/บริการลานตากข้าว
ให้แจ้งจังหวัดและอำเภอด้วย" — ขยายข้อบังคับแจ้งจังหวัด/อำเภอ (รอบที่ 3
ด้านล่าง) จากเดิม 4 ประเภทให้ครอบคลุม**ทุกประเภทธุรกิจที่สมัครเองได้**
(เพิ่ม Lender, VillageFund, DryingYardService เข้ามาด้วย)

**ไม่ต้องสร้าง/รัน migration ใหม่** — คอลัมน์ `region_code`/`district_code`
ใน `identity.organization` จาก `grant_org_district.sql` (รอบที่ 3) รองรับ
ทุกประเภทธุรกิจอยู่แล้วตั้งแต่แรก (เป็น nullable ไม่ได้ผูก CHECK constraint
กับ org_type ใดๆ) — การขยายสโคปนี้แก้แค่ฝั่งโค้ด: `ORG_TYPES_REQUIRING_
LOCATION` ใน `backend/src/routes/auth.js` เปลี่ยนจากลิสต์ 4 ประเภทเป็น
`= ORG_SELF_REGISTER_TYPES` ตรงๆ (เท่ากับทุกประเภทที่สมัครเองได้) และฝั่ง
`frontend/js/register-provider.js` เปลี่ยนเป็น `Object.keys(ORG_TYPE_LABEL)`
ผลคือฟอร์มสมัคร (`register-provider.html`) จะโชว์ช่องจังหวัด/อำเภอทันทีที่
เลือกประเภทธุรกิจใดๆ ก็ได้ในดรอปดาวน์ ไม่มีประเภทไหนได้รับการยกเว้นอีกแล้ว

หากไม่ได้รัน `grant_org_district.sql` (รอบที่ 3) ไว้ก่อนหน้านี้ ต้องรันก่อน
ฟีเจอร์นี้จึงจะใช้งานได้ครบทุกประเภท — ถ้ารันไปแล้วไม่ต้องทำอะไรเพิ่มฝั่ง
ฐานข้อมูล

**เพิ่มเมื่อ 2026-09-06 (รอบที่ 3):** `grant_org_district.sql` — ตามคำขอ
"ให้ผู้รับซื้อผลผลิต ผู้ให้บริการเครื่องจักรกล ผู้ให้บริการขนส่งลงทะเบียนแจ้ง
จังหวัดและอำเภอด้วย" ตามด้วยข้อความถัดมา "ผู้ขายปัจจัยการผลิตด้วย" — สรุปคือ
ต้องการให้ 4 ประเภทธุรกิจนี้แจ้งจังหวัด+อำเภอตอนสมัคร: **ผู้รับซื้อผลผลิต
(Buyer), ผู้ให้บริการเครื่องจักรกล (MachineryService), ผู้ให้บริการขนส่ง
(Logistics), ผู้ขายปัจจัยการผลิต (InputSupplier)** — ประเภทอื่นที่สมัครเองได้
(ผู้ปล่อยกู้/Lender, กองทุนหมู่บ้าน/VillageFund, บริการลานตากข้าว/
DryingYardService) ไม่ได้ถูกระบุในคำขอทั้งสองข้อความ จึงไม่บังคับ

เพิ่มคอลัมน์ใหม่ `region_code`/`district_code` ใน `identity.organization`
(nullable — องค์กรที่สมัครไว้ก่อนหน้านี้ไม่มีข้อมูลนี้) รูปแบบเดียวกับ
`identity.farmer.region_code`/`district_code` ทุกประการ (ข้อความล้วน ไม่มี
FK/ตารางอ้างอิง ตรวจสอบแค่ฝั่ง client ผ่าน TH_PROVINCES/TH_DISTRICTS) — เป็น
คนละอย่างกับ `partner.vendor_profile.service_regions` เดิม (รายการหลาย
จังหวัดที่ผู้ให้บริการประกาศว่า "ให้บริการครอบคลุมพื้นที่ไหนบ้าง") อันนี้คือ
"ธุรกิจนี้ตั้งอยู่จังหวัด/อำเภอไหน" ค่าเดียว

ฟอร์มสมัครผู้ให้บริการ (`frontend/register-provider.html` — ฟอร์มเดียวใช้
ร่วมกันทุกประเภทธุรกิจ) จะโชว์/ซ่อนช่องจังหวัด+อำเภอทันทีเมื่อเลือกประเภท
ธุรกิจในดรอปดาวน์ (ไม่ใช่ตรวจตอนกดส่งอย่างเดียว) — เลือกประเภทที่ไม่เกี่ยว
(เช่น ผู้ปล่อยกู้) จะไม่เห็นช่องนี้เลย ฝั่ง backend
(`POST /auth/org-register`) ตรวจซ้ำอีกชั้นเสมอ (คืน `400
missing_required_fields` ถ้า 4 ประเภทข้างต้นไม่กรอกจังหวัด/อำเภอ)
...
**เพิ่มเมื่อ 2026-09-13 (ค่ำ):** `grant_about_content_add_farmer_section.sql` —
ตามคำขอเพิ่มการ์ด "เกษตรกร" ในหน้า "เกี่ยวกับเรา" (`frontend/about.html`) เป็น
ลำดับที่ 2 (ต่อจากการ์ด "เกี่ยวกับ AgroLink" และก่อนการ์ด "ผู้ปล่อยกู้ (Lender)")
เนื้อหาการ์ดนี้มาจากตาราง `content.about_section` (ดู `grant_about_content.sql`
เดิม) ซึ่งมีช่องว่างของ `display_order` เผื่อไว้ตั้งแต่ต้น (0, 20, 30, 40, ...)
migration นี้จึงแค่ insert แถวใหม่ 1 แถวด้วย `display_order = 10` ให้เข้าไปแทรก
ตรงกลางพอดี ไม่ต้องแก้/เรียงลำดับแถวเดิมเลย ไม่มีตาราง/คอลัมน์ใหม่ ไม่มีการแก้
โค้ด backend/frontend ใดๆ (หน้า `about.html` ดึงข้อมูลจาก `GET /about` แบบไดนามิก
อยู่แล้ว) กันการรันซ้ำด้วยเงื่อนไข "insert เฉพาะเมื่อยังไม่มีแถวชื่อ 'เกษตรกร'"

**หมายเหตุสำคัญ:** migration นี้ยังไม่ได้รันกับฐานข้อมูลจริงบน Render —
ต้องรันเองผ่าน psql ตามขั้นตอนใน DEPLOY.md ข้อ 5-7 ก่อนการ์ดนี้จะปรากฏบนหน้า
"เกี่ยวกับเรา" จริง เช่นเดียวกับทุก migration ก่อนหน้า
...
รับซื้อได้และจำกัดจำนวนการรับซื้อได้สำหรับราคานั้น และเมื่อซื้อได้ครบจำนวน
แล้วก็ไม่สามารถรับซื้อได้อีก ต้องตั้งประกาศใหม่ (ใช้สำหรับผู้ซื้อต้องการ
เร่งซื้อผลผลิตเข้าสต๊อกในช่วงเวลาสั้นๆ)" พร้อมข้อกำหนดเพิ่มเติมจากผู้ใช้
(ข้อความถัดมา): "ให้แจ้งด้วยว่าราคาที่ประกาศนี้รับซื้อเฉพาะรายการขายผ่าน
ระบบออนไลน์นี้เท่านั้น ไม่สามารถไปอ้างอิงขายที่หน้าลานหรือหน้าโรงงานได้"

ก่อนเริ่มเขียนโค้ดได้ถามผู้ใช้ 2 คำถามเพื่อกำหนดสโคป: (1) ขอบเขตสินค้า —
เลือก **ทุกชนิดสินค้าเกษตร** (registry.commodity_ref) ไม่จำกัดแค่ข้าว
ต่างจากระบบ "ราคารับซื้อข้าวประจำวัน" (buy_price_quote) เดิมที่มีอยู่แล้ว
ซึ่งเป็นกระดานราคายืนพื้นไม่จำกัดจำนวนและใช้กับข้าวเท่านั้น — เป็นคนละ
ฟีเจอร์กัน ไม่ได้แก้ของเดิม; (2) กลไกการขาย — เลือก **ตัดจำนวนทันทีเมื่อ
เกษตรกรกดขาย ไม่ต้องรอผู้ซื้อกดยืนยัน** เพื่อให้สมกับความต้องการ "เร่งซื้อ"
ของผู้ซื้อ โดยยังคงใช้ pipeline การส่งมอบ/ตรวจคุณภาพ/ชำระเงินเดิมที่มีอยู่
แล้ว (produce.delivery, POST /buyer/deliveries/:id/confirm-quality,
/settle) ไม่ได้สร้างระบบชำระเงินใหม่ — ระบบใหม่นี้ทำแค่ขั้นตอน "ล็อกราคา/
จำนวนที่จะขาย" ให้อัตโนมัติเท่านั้น

เพิ่มตารางใหม่ 2 ตัว (`marketplace.buy_campaign`,
`marketplace.buy_campaign_order`) และฟังก์ชัน
`marketplace.sell_into_buy_campaign()` ที่ล็อกแถวประกาศด้วย `FOR UPDATE`
กันการขายเกินจำนวนคงเหลือเมื่อมีเกษตรกรหลายรายขายพร้อมกัน แล้วเรียกใช้
`produce.record_delivery()` เดิมสร้างรายการส่งมอบตามปกติ — เมื่อขายครบ
ตามจำนวนที่ตั้งไว้ ระบบจะเปลี่ยนสถานะประกาศเป็น `closed` ให้อัตโนมัติทันที
(ปิดแล้วไม่สามารถเปิดใหม่ได้ ต้องตั้งประกาศใหม่ ตรงตามที่ผู้ใช้ขอ)

หมายเหตุคำเตือนเรื่องราคาใช้ได้เฉพาะระบบออนไลน์ ใส่เป็นข้อความ UI ที่แสดง
ทั้งฝั่งฟอร์มเปิดประกาศของผู้ซื้อ (frontend/buyer/dashboard.html) และหน้า
เรียกดูของเกษตรกร (frontend/buy-campaigns.html หน้าใหม่) — เป็นข้อความ
ล้วนๆ ไม่มีคอลัมน์ในฐานข้อมูลเกี่ยวข้อง เพราะไม่มีอะไรให้บังคับใช้เกี่ยวกับ
การตกลงซื้อขายนอกระบบได้

เพิ่มปุ่ม "ปิดประกาศนี้ก่อนกำหนด" ฝั่งผู้ซื้อ (ปิดเองได้ก่อนเต็มจำนวน เช่น
กรอกราคาผิด) — เป็นส่วนเสริมเล็กน้อยที่ไม่ได้ถูกขอตรงๆ แต่เพิ่มให้เพราะมี
ประโยชน์ชัดเจนและความเสี่ยงต่ำ ปิดแล้วพฤติกรรมเหมือนปิดอัตโนมัติทุกประการ
(เปิดใหม่ไม่ได้เช่นกัน)
