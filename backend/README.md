...
- `GET /farmer/input-suppliers?province_code=` → every Verified `InputSupplier` organization, with how many active products it currently has listed — a small supporting directory endpoint, same shape as `GET /farmer/lenders`, so the frontend never has to hardcode an `org_id`. Optional `province_code` (added 2026-08-29, an ISO 3166-2:TH code) narrows to suppliers whose `partner.vendor_profile.service_regions` includes it — an org with an empty `service_regions` (the default, until it uses the new "พื้นที่ให้บริการ" section on its own dashboard) always matches, treated as serving every province.
- `GET /farmer/products?category=&org_id=&province_code=` → browse the ACTIVE catalog (`is_active = true` only) across every Verified InputSupplier, or one via `org_id`, or narrowed to a province via `province_code` (same semantics as above), joined with the supplier's `org_name`. See "Farmer ordering flow" below. `province_code` closes a real pre-existing gap: `marketplace.html`'s จังหวัด filter dropdown existed in the HTML for a while but was never wired to anything (no options, no query param) until this addition, added together with the matching `GET`/`PUT /inputsupplier/service-regions` frontend UI (previously the routes existed but had no UI, so `service_regions` sat unused).
