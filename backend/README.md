...
- `POST /auth/register` — body `{ "full_name", "phone", "national_id", "region_code", "district_code" }` → creates a new `identity.farmer` row (status `pending_kyc`), grants it the `farmer.self` role in `identity.subject_role`, mints a fresh mock OIDC claim (`oidc|farmer-<uuid>`), and auto-issues a session JWT so the new farmer lands straight in the portal. `national_id` is SHA-256 hashed before it ever reaches the database — only the hash is stored. `district_code` was added 2026-09-06 (`grant_farmer_district.sql`) alongside the pre-existing `region_code` — same free-text convention (no FK/lookup table; validated client-side only against `TH_DISTRICTS` in `frontend/js/districts.js`), and required here even though the underlying column is nullable (existing farmers registered before this column existed have no district on file). Duplicate phone/national ID return `409` with `phone_already_registered` / `national_id_already_registered`.
...
**Support Chat** (`src/routes/support.js`, mounted at `/support` — subject-type agnostic like `procurement.js`/`farmer360.js`, any JWT subject type EXCEPT `platform`) — added 2026-08-30, see `grant_support_chat.sql` for the schema and the user request that scoped it (web widget first — a floating chat button/panel added to every non-admin portal's `dashboard.html` via the single shared `frontend/js/support-widget.js` script; LINE OA and voice messages both explicitly deferred — LINE needs a real LINE Developers channel + a publicly reachable webhook host, voice needs an external speech-to-text API key, neither available in this sandbox). One continuous conversation per subject (`support.conversation`, unique on `(subject_type, subject_id)`) rather than a ticket-per-issue system — a returning user always lands back in the same thread.
- `GET /support/messages` → the calling subject's own conversation + full message history (`{ conversation: null, messages: [] }` if they've never messaged). Marks it read BY THE USER as a side effect — the widget calls this for both its initial load and its ~8s poll for admin replies.
- `POST /support/messages` — body `{ message }` → get-or-create (upsert) the caller's conversation and append a `sender_role='user'` row, flagging `unread_by_admin` so it surfaces on the admin inbox below.
- The admin (platform) side — listing every conversation, reading a thread, replying — lives under `/admin/support/*` in `admin.js` instead (see below), not in this file, per this project's "platform-only endpoints live in admin.js" convention.
...
- `GET /farmer/input-suppliers?province_code=&district_code=` → every Verified `InputSupplier` organization, with how many active products it currently has listed — a small supporting directory endpoint, same shape as `GET /farmer/lenders`, so the frontend never has to hardcode an `org_id`. Optional `province_code` (added 2026-08-29, an ISO 3166-2:TH code) narrows to suppliers whose `partner.vendor_profile.service_regions` includes it, either as a bare province code ("serves the whole province") or as a matching district code; an org with an empty `service_regions` (the default, until it uses the new "พื้นที่ให้บริการ" section on its own dashboard) always matches, treated as serving every province. Optional `district_code` (added 2026-08-29, later same day, `<province_code>-NN` — see `frontend/js/districts.js`'s doc comment for this scheme's provenance caveat) further narrows to suppliers who declared either that specific district or the whole parent province; passing `district_code` without `province_code` is ignored (the frontend always sends them together). Both params are implemented by a shared `serviceRegionCondition()` helper duplicated in `farmer.js` and `farmermachinery.js`, per this project's per-file convention.
- `GET /farmer/products?category=&org_id=&province_code=&district_code=` → browse the ACTIVE catalog (`is_active = true` only) across every Verified InputSupplier, or one via `org_id`, or narrowed to a province/district via `province_code`/`district_code` (same semantics as the identically-named params on `GET /farmer/input-suppliers` above), joined with the supplier's `org_name`. See "Farmer ordering flow" below. `province_code` closes a real pre-existing gap: `marketplace.html`'s จังหวัด filter dropdown existed in the HTML for a while but was never wired to anything (no options, no query param) until this addition, added together with the matching `GET`/`PUT /inputsupplier/service-regions` frontend UI (previously the routes existed but had no UI, so `service_regions` sat unused). `district_code` (added 2026-08-29, later same day) is the same feature extended one level down, with a matching `<select id="districtFilter">` on `marketplace.html` that cascades off the selected province.
...
- `GET`/`PUT /machinery/service-regions` (added 2026-08-29) — read/replace this org's declared coverage (`partner.vendor_profile.service_regions`, a `text[]` holding a free mix of bare ISO 3166-2:TH province codes and `<province_code>-NN` district codes — see `frontend/js/districts.js`'s doc comment for the district scheme's provenance caveat). Same shape as `GET`/`PUT /inputsupplier/service-regions`, and closes the identical pre-existing gap: `GET /farmer/machinery-providers?province_code=&district_code=` filters on this column, but until this pair of routes existed nothing anywhere ever let a machinery/drying-yard org SET it — an org with an empty `service_regions` (the default) is treated as serving every province.
- `GET /farmer/machinery-providers?service_type=&province_code=&district_code=` → browse Verified machinery/drying-yard providers' priced rate-card lines, optionally filtered by `service_type` (one of the seven `RATE_CARD_ITEMS` service_type buckets) and/or `province_code`/`district_code` (added 2026-08-29, same semantics and shared `serviceRegionCondition()` helper as `GET /farmer/input-suppliers` above). Backs `machinery-marketplace.html`'s filter bar, including its `provinceFilter`/`districtFilter` dropdowns.
...
- `GET /admin/support/conversations` (added 2026-08-30, see the "Support Chat" section above) — every conversation across every eligible subject type, newest activity first, with a resolved human-readable `subject_label` (farmer/org/staff-member/officer name — same `COALESCE(...)`-over-`LEFT JOIN`s pattern `procurement.js` uses for its own polymorphic `requester_subject_type`/`requester_subject_id`, extended to all four subject types this feature supports).
- `GET /admin/support/conversations/:id/messages` — full thread, oldest first. Marks it read BY THE ADMIN as a side effect.
- `POST /admin/support/conversations/:id/reply` — body `{ message }` → appends a `sender_role='admin'` message and flags `unread_by_user` so it surfaces back on that subject's own widget. Backs the new "💬 ข้อความสนับสนุน" section on `frontend/admin/dashboard.html`.
...
card**: exactly eleven possible line items (`plow_rough`, `spraying`,
`drying`, `straw_pelletizing`, `straw_baling`, `laser_land_leveling`, `machinery_rental`, ...), one row per
