/**
 * AgroLink MarketVenue (เจ้าของสถานที่จำหน่ายสินค้า) Portal — shared API client.
 *
 * Same shape as ../../inputsupplier/js/api.js and the other organization
 * portals' own copies — its own localStorage key so a lender/buyer/
 * machinery/inputsupplier/marketvenue session in the same browser never
 * collide, and redirect targets point at this folder's own pages.
 */
...
const AUTH_STORAGE_KEY = "agrolink_marketvenue_session";

const AgroLinkMarketVenueAPI = (() => {
  function getSession() {
   ...
   * are a REAL marketvenue-org token just not (yet) approved — keeps the
  ...
  async function request(path, options = {}) {
    ...
    if (res.status === 403) {
      ...
      clearSession();
      window.location.href = "index.html?reason=not_a_market_venue_org";
      throw new Error("not_a_market_venue_org");
    }
