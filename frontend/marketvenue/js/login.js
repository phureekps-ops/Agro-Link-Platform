...
async function doLogin(claim) {
  ...
  try {
    await AgroLinkMarketVenueAPI.login(claim);
...
} else if (params.get("reason") === "not_a_market_venue_org") {
  showError("บัญชีนี้ไม่ใช่บัญชีผู้ให้บริการพื้นที่ขายสินค้าเกษตร กรุณาเข้าสู่ระบบด้วยบัญชีที่ถูกต้อง");
}

// If already logged in, skip straight to the dashboard.
if (AgroLinkMarketVenueAPI.getSession()) {
  window.location.href = "dashboard.html";
}
