...
async function doLogin(claim) {
  ...
  try {
    await AgroLinkCoopAPI.login(claim);
...
} else if (params.get("reason") === "not_a_cooperative") {
  showError("บัญชีนี้ไม่ใช่บัญชีสหกรณ์ กรุณาเข้าสู่ระบบด้วยบัญชีที่ถูกต้อง");
}

// If already logged in, skip straight to the dashboard.
if (AgroLinkCoopAPI.getSession()) {
  window.location.href = "dashboard.html";
}
