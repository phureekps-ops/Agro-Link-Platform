...
async function doLogin(claim) {
  ...
  try {
    await AgroLinkInputSupplierAPI.login(claim);
...
} else if (params.get("reason") === "not_an_input_supplier_org") {
  showError("บัญชีนี้ไม่ใช่บัญชีผู้จำหน่ายปัจจัยการผลิต กรุณาเข้าสู่ระบบด้วยบัญชีที่ถูกต้อง");
}

// If already logged in, skip straight to the dashboard.
if (AgroLinkInputSupplierAPI.getSession()) {
  window.location.href = "dashboard.html";
}
