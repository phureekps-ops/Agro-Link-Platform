/**
 * AgroLink Fertilizer-Mixing Service Portal — shared API client.
 *
 * Same shape as ../../machinery/js/api.js — kept as its own copy for the
 * same reason every other portal's api.js is separate: the storage key
 * must be different so a session in this portal never collides with a
 * farmer/lender/buyer/machinery/etc. session in the same browser, and the
...
const AUTH_STORAGE_KEY = "agrolink_fertilizermixing_session";

const AgroLinkFertilizerMixingAPI = (() => {
  function getSession() {
   ...
   * fertilizer-mixing login endpoint was needed on the backend.
   */
   ...
   *   - 'kyb_not_verified': a REAL fertilizer-mixing-org token, just not
   *     yet approved (e.g. freshly self-registered via
   ...
   *   - anything else (e.g. a farmer token, or a non-fertilizer-mixing
  ...
  async function request(path, options = {}) {
    ...
    if (res.status === 403) {
      ...
      clearSession();
      window.location.href = "index.html?reason=not_a_fertilizermixing_org";
      throw new Error("not_a_fertilizermixing_org");
    }
