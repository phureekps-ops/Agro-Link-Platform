...
async function doLogin(claim) {
  ...
  try {
    await AgroLinkFertilizerMixingAPI.login(claim);
...
} else if (params.get("reason") === "not_a_fertilizermixing_org") {
  showError("บัญชีนี้ไม่ใช่บัญชีผู้ให้บริการผสมปุ๋ยสั่งตัด กรุณาเข้าสู่ระบบด้วยบัญชีที่ถูกต้อง");
}

// If already logged in, skip straight to the dashboard.
if (AgroLinkFertilizerMixingAPI.getSession()) {
  window.location.href = "dashboard.html";
}
