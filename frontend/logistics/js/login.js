...
async function doLogin(claim) {
  ...
  try {
    await AgroLinkLogisticsAPI.login(claim);
...
} else if (params.get("reason") === "not_a_logistics_org") {
  showError("บัญชีนี้ไม่ใช่บัญชีผู้ให้บริการขนส่ง กรุณาเข้าสู่ระบบด้วยบัญชีที่ถูกต้อง");
}

// If already logged in, skip straight to the dashboard.
if (AgroLinkLogisticsAPI.getSession()) {
  window.location.href = "dashboard.html";
}
