...
async function doLogin(claim) {
  ...
  try {
    await AgroLinkGovAPI.login(claim);
...
} else if (params.get("reason") === "not_a_government_officer") {
  showError("บัญชีนี้ไม่ใช่บัญชีเจ้าหน้าที่ภาครัฐ กรุณาเข้าสู่ระบบด้วยบัญชีที่ถูกต้อง");
}

// If already logged in, skip straight to the dashboard.
if (AgroLinkGovAPI.getSession()) {
  window.location.href = "dashboard.html";
}
