...
async function doLogin(claim) {
  ...
  try {
    await AgroLinkVillageFundAPI.login(claim);
...
} else if (params.get("reason") === "not_a_villagefund") {
  showError("บัญชีนี้ไม่ใช่บัญชีกองทุนหมู่บ้าน กรุณาเข้าสู่ระบบด้วยบัญชีที่ถูกต้อง");
}

// If already logged in, skip straight to the dashboard.
if (AgroLinkVillageFundAPI.getSession()) {
  window.location.href = "dashboard.html";
}
